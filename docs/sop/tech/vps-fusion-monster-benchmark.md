...
| 机器 | 大洲 | 主要定位 | 性能 | 国内访问 | IP 质量 | 总评 |
|---|---|---|---:|---:|---:|---|
| 不知名小厂香港月付 1.88 2C2G（已倒闭） | 亚洲 | 香港低价机器基础测试 | 未测完整性能 | 很强 | 好 | 延迟漂亮，带宽明显卡在 30Mbps 左右，服务商已跑路 |
| WAWO 香港年付 39 1C0.5G | 亚洲 | 香港低价小鸡基础测试 | 弱 | 中上偏弱 | 好 | 本地带宽充足，移动走 CMIN2 延迟低，电信联通绕远，IP 干净但性能弱 |
| ZOUTER 香港月付 9.9 1C1G | 亚洲 | 香港小规格机器基础测试 | 强 | 中等偏差 | 一般 | 性能漂亮，但电信联通绕美国回国，只有移动方向延迟可用 |
| Azure 免费 1C1G 日本 | 亚洲 | 日本免费机基础测试 | 一般 | 中上 | 一般 | 联通和部分电信可用，磁盘很弱 |
| lowsla 日本东京 NAT 小杯机 | 亚洲 | 日本 NAT 小杯机基础测试 | 未测完整性能 | 中上 | 一般 | 三网普通线路但延迟低，IP 不算脏，邮件端口外连基本全封 |
| Google 永久免费美国 Iowa | 北美洲 | 免费保号/干净 IP | 弱 | 一般 | 好 | 和 Oregon 类似，免费自用可以，别指望性能 |
| Google 永久免费美国 Oregon | 北美洲 | 免费保号/干净 IP | 弱 | 一般 | 好 | CPU/IO 很弱，但 Google IP 相对干净 |
| DO 美国 1C0.5G | 北美洲 | 美国小规格机器基础测试 | 中等 | 弱 | 一般 | 本地测速好，回国体验差 |
| Cloudnium 美国月付 3 2C2G | 北美洲 | 美国中规格机器基础测试 | 中等 | 弱 | 一般 | 配置够用，三网普通线路回国延迟高，移动方向基本不可用 |
| Raksmart 美国 4837 月付 14 2C4G | 北美洲 | 美国 4837 线路机器基础测试 | 中等偏上 | 中等 | 一般 | 三网全走 4837，联通方向体验最好，电信一般、移动差 |
| DO 加拿大 1C0.5G | 北美洲 | 加拿大小规格机器基础测试 | 中等 | 弱 | 差 | 盘不错，但 IP 质量和国内线路都不理想 |
| uzuma 加拿大 CAMT1.Small | 北美洲 | 加拿大小规格机器基础测试 | 未测完整性能 | 弱 | 一般偏差 | Leaseweb Montréal，回国绕路，IP 检测结果分裂 |
| lowsla 荷兰阿姆斯特丹 NAT 小杯机 | 欧洲 | 荷兰 NAT 小杯机基础测试 | 未测完整性能 | 弱 | 好 | IP 干净，但路由远、回国延迟高 |
| DO 澳大利亚 1C0.5G | 大洋洲 | 澳洲小规格机器基础测试 | 强 | 很弱 | 差 | 跑分好看，国内方向很弱 |
...
&emsp;&emsp;WAWO 香港，`1C / 458MiB / 3.80GiB`，CPU 为 `Intel Xeon E3-12xx v2`，单核 sysbench `398`，仍是这批里最弱的一档。内存只有 `458MiB`，读写约 `7552/5963MB/s`。磁盘 fio 测试失败，1GB dd 写入约 `538MB/s`、读取约 `355MB/s`，小盘机器特征明显。

#### 网络与 IP 质量

&emsp;&emsp;回程是混合线路：北京/上海/成都电信都识别到 `CTGNET` 精品线路，广州电信为 `CTGNET / 163`；联通走 `4837` 普通线路；移动同时识别到 `CMI` 普通和 `CMIN2` 精品。本地 Speedtest `422/992Mbps`，香港本地 `244/1019Mbps`，本地带宽比上一轮测试明显更充足；但国内方向依旧反差大：广州移动经 CMIN2 直连约 `16ms`，广州电信要经香港 CN2 绕回广州约 `163ms`，广州联通经 CUG 绕回约 `182ms`。国内测速联通上海 5G 下载只有 `2.13Mbps`、联通 Beijing 更低到 `0.69Mbps`，移动 Suzhou 下载 `0.57Mbps`。

&emsp;&emsp;IP 质量很好：信任分 `100`，VPN/代理分 `0`，欺诈分 `0`，DNS 黑名单 `0`，但 Google 搜索可行性为 `NO`。

#### 测评

&emsp;&emsp;这台的关键矛盾在路由：标称香港，本地带宽也不错，但电信和联通回国都绕远，只有移动走 CMIN2 延迟低。配合很弱的 CPU 和内存，它更适合对 IP 质量有要求、低负载、且主要走移动方向的轻量任务，不适合做电信联通主力或高负载服务。

#### 原始融合怪结果

::: details 点击展开 PGrQ6.txt
<<< ../../../otherdocs/vps测评数据/WAWO香港年付39 1-0.5/PGrQ6.txt
:::
...
&emsp;&emsp;IP 质量一般：信任分 `34`，VPN 分 `99`，欺诈分 `0`，DNS 黑名单 `8` 条，Google 搜索可行性为 `NO`。

#### 测评

&emsp;&emsp;典型的"本地性能漂亮、国内方向瘸腿"。CPU 和磁盘跑分很强，但电信联通绕美国回国延迟很高，只有移动方向延迟可用。适合对单核性能和磁盘有要求的轻量任务，不适合做电信联通方向的国内入口。
...
&emsp;&emsp;IP 质量一般：信任分 `33`，欺诈分 `65`，滥用分 `0`，ASN 滥用分偏高，Google 搜索可行性为 `NO`。

#### 测评

&emsp;&emsp;配置够用，但回国全是普通线路，延迟高、移动方向基本不可用。它更像一台北美本地轻量机，对国内访问稳定性要求高的场景不合适。
...
&emsp;&emsp;IP 质量一般：信任分 `33`，欺诈分 `65`，滥用分 `0`，DNS 黑名单 `7` 条，Google 搜索可行性为 `NO`。

#### 测评

&emsp;&emsp;全 4837 回程让它对联通用户最友好，电信方向延迟也能接受，但移动方向基本不可用。适合联通为主的国内访问场景，或对 4837 线路有偏好的用途，不适合移动用户或对 IP 信誉要求高的业务。
